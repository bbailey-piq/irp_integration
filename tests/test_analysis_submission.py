"""
Tests for portfolio analysis job submission.

An analysis may legitimately have no tags to apply. ``treatyIds`` and ``tagIds``
go into the same ``settings`` dict, so an empty ``tagIds`` is as valid as the
empty ``treatyIds`` the package already sends: the tag lookup is skipped and the
job is submitted with ``tagIds: []``.
"""
...
# softwareVersionCode contains "HD", so classify_model_profile returns "HD" and no
# event rate scheme is required. That keeps the event rate scheme request out of
# the queue.
MODEL_PROFILE_BODY = {
    "count": 1,
    "items": [{
